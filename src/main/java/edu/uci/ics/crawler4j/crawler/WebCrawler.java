 ...
 *File: edu.uci.ics.crawler4j.crawler.WebCrawler.java <2015年4月13日>
 ****************************************************************
...
public class WebCrawler implements Runnable {
	 ...
	 * 请求异常URL
	 */
	static final Map<String, Integer> mapErrorUrl = new HashMap<String, Integer>();
	
	/**
	 * 默认不爬不能解析的格式，如音视频
	 */
	protected static final Pattern LIMIT_FILTERS = Pattern.compile(".*(\\.(css|js|bmp|gif|jpe?g" + "|png|tiff?|mid|mp2|mp3|mp4"  
	...
	protected void onPageBiggerThanMaxSize(String urlStr, long pageSize) {
		logger.warn(
				"Skipping a URL: {} which was bigger ( {} ) than max allowed size",
				urlStr, pageSize);
	}
	...
	protected void onUnexpectedStatusCode(String urlStr, int statusCode,
		...
		logger.warn("Skipping URL: {}, StatusCode: {}, {}, {}", urlStr,
		...
		try {
			...
			handlePageStatusCode(curURL, statusCode,
					EnglishReasonPhraseCatalog.INSTANCE.getReason(statusCode,
							Locale.ENGLISH)); // Finds the status reason for all
												// known statuses
			...
			if (statusCode != HttpStatus.SC_OK) { // Not 200
				if (statusCode == HttpStatus.SC_MOVED_PERMANENTLY
						...
						|| statusCode == 308) { // is 3xx todo follow
					...
					}else if(fetchResult.getStatusCode()==429){
						reprocessPage("429", curURL);
					}
				}
	...
	private void reprocessPage(String status, WebURL curURL) {
		...
		try {
			...
			if(status.equalsIgnoreCase("timeout")){//请求超时
				...
				infoThread = 60;
			}else if(status.equalsIgnoreCase("429")){
				infoBody = "StatusCode=429";
				infoThread = 2;
			}else{
				return;
			}
